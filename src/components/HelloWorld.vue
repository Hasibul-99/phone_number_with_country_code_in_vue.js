...
export default {
  ...
  watch: {
    state(value) {
      if (value && this.mode !== 'prefix') {
        ...
        if (this.numberFormat) this.phone = this.formattedResult;
      }
    },
    value() {
      this.phone = this.value;
  ...
  methods: {
    phoneNumberValidaton() {
      ...
      let evt = event && event.KeyboardEvent ? event.KeyboardEvent : window.event,
          charCode = evt && evt.which ? evt.which : evt.keyCode;
    ...
    onInput() {
        ...
        // Emit the respone item value format number

        let formatedNumber = "";

        if (this.response && this.response.number && this.response.country && this.response.country.dialCode) {
              let number = this.response.number,
                  dialCode = this.response.country.dialCode;

          if (this.response.number[0] !== '+') {
            formatedNumber = "+".concat(dialCode, number);
          } else {
            formatedNumber = number.replace(/\s/g, "");
          }
        }

        this.$emit('formatedNumber', formatedNumber)
    },
    initializeCountry() {
      /**
